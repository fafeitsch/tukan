...
type Parameters struct {
	...
	// We don't include the admin password for security reasons and because we can't restore it properly
	...
	for index := 0; index < structValue.Elem().NumField(); index++ {
		...
		if value, ok := raw[jsonName]; ok {
			...
			case reflect.Int:
				...
				if !ok {
					// sometimes, it's string for whatever reason
					convertedString, ok := value.(string)
					if !ok {
						return fmt.Errorf("json property \"%s\" has wrong type, want \"float64\" or \"string\", found \"%v\"", jsonName, reflect.TypeOf(value))
					}
					converted, err = strconv.ParseFloat(convertedString, 64)
					if err != nil {
						return fmt.Errorf("json property \"%s\" has wrong type, want \"float64\" or \"string\", found \"%v\"", jsonName, reflect.TypeOf(value))
					}
				}
